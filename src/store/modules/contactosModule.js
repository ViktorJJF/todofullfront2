import api from "@/services/api/contactos";
...
const module = {
  ...
  mutations: {
    ...
    update(state, { id, data }) {
      let indexToUpdate = state.contactos.findIndex(
        (member) => member._id == id
      );
      state.contactos.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state, id) {
      let indexToDelete = state.contactos.findIndex(
        (member) => member._id == id
      );
