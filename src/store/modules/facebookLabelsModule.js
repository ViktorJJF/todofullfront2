...
const module = {
  ...
  mutations: {
    list(state, data) {
      state.facebookLabels = data;
    },
    create(state, data) {
      state.facebookLabels.unshift(data);
    },
    ...
    delete(state, id) {
      let indexToDelete = state.facebookLabels.findIndex(
        (member) => member._id == id
      );
      state.facebookLabels.splice(indexToDelete, 1);
    },
  },
  getters: {
    getLabelsByCategoryId: (state) => (categoryId) => {
      return state.facebookLabels.filter(
